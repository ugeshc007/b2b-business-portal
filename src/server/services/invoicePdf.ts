...
const invoiceTerms = [
  "Agreed payment terms to be strictly followed.",
  "Manufacturer's warranty as per terms in Manufacturer's Policy.",
  "In case of APPLE products, warranty will be covered directly by manufacturer.",
  "The recipient of \"Electronic devices\" is required to account for tax under RCM, with ref. to UAE VAT Law read with CD 91/2023 as applicable.",
  "Buyer under Article 51 of VAT regulation confirms the goods bought are for further sale outside UAE or to UAE, Mainland Company and will not be consumed or used in the free zone.",
  "Buyer undertakes to inform and pay due VAT if there is any change regarding the usage / consumption of goods as mentioned above.",
  "The products listed on this invoice are intended solely for use and sale within non-sanctioned countries.",
];

function drawInvoiceTerms(doc: PDFKit.PDFDocument, x: number, y: number, width: number, color = "#111111", fontSize = 6.4) {
  doc
    .font("Helvetica")
    .fontSize(fontSize)
    .fillColor(color)
    .text(invoiceTerms.join("\n"), x, y, { width, lineGap: 1.5 });
}
...
function drawBuy2dayInvoicePdf(doc: PDFKit.PDFDocument, invoice: InvoicePdfData) {
  ...
  drawInvoiceTerms(doc, 42, bankY + 126, 508);
...
export async function generateInvoicePdf(invoiceId: string) {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    drawInvoiceTerms(doc, 42, 704, 508, brand.text, 6.6);
    doc.fillColor(brand.text).font("Helvetica").fontSize(8).text("Generated by B2B Business Portal. This PDF copy is stored for audit and email attachment use.", 42, 790, { width: 508 });
